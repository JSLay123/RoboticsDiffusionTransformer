...
def serialize_example(action, qpos, cam_high, cam_wrist, instruction, terminate_episode):
    feature = {
        ...
        'cam_wrist': _bytes_feature(tf.io.serialize_tensor(cam_wrist)),
    ...
    try:
        with h5py.File(filepath, 'r') as f, tf.io.TFRecordWriter(tfrecord_path) as writer:
            ...
            for i in range(first_idx-1, num_episodes):
                ...
                cam_wrist = f['observations']['images']['cam_wrist'][i]
                # cam_left_wrist = f['observations']['images']['cam_left_wrist'][i]
                # cam_right_wrist = f['observations']['images']['cam_right_wrist'][i]
                # instruction  = f['instruction'][()]
                instruction = "pick and place the cube".encode("utf-8")
                terminate_episode = i == num_episodes - 1
                serialized_example = serialize_example(action, qpos, cam_high, cam_wrist, instruction, terminate_episode)
...
def write_tfrecords(root_dir, out_dir):
    ...
    hdf5_files = []
    # 递归遍历root_dir下的所有目录和文件
    #root: 当前正在遍历的目录路径；dirs: 当前目录下的子目录列表；files: 当前目录下的文件列表
...
root_dir = "/home/silei/WorkSpace_git/RoboticsDiffusionTransformer/data/datasets/my_franka/rdt_data/"
out_dir = "/home/silei/WorkSpace_git/RoboticsDiffusionTransformer/data/datasets/my_franka/tfrecords/"

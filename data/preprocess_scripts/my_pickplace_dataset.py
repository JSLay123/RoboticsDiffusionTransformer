...
def _parse_function(proto, precomputed_instr_embed_path):
    # tf.io.FixedLenFeature([], tf.string): 固定长度的字符串特征，这种设计是为了高效存储和读取
    keys_to_features = {
        ...
        'cam_wrist': tf.io.FixedLenFeature([], tf.string),
    ...
    parsed_features = tf.io.parse_single_example(proto, keys_to_features)   # 从序列化数据中提取特征字典
    ...
    cam_wrist = tf.io.parse_tensor(parsed_features['cam_wrist'], out_type=tf.string)
    ...
    cam_wrist = tf.image.decode_jpeg(cam_wrist, channels=3, dct_method='INTEGER_ACCURATE')
    ...
    cam_wrist = tf.reverse(cam_wrist, axis=[-1])
    # cam_left_wrist = tf.reverse(cam_left_wrist, axis=[-1])
    # cam_right_wrist = tf.reverse(cam_right_wrist, axis=[-1])

    action = tf.reshape(action, [8])
    # base_action = tf.reshape(base_action, [2])
    qpos = tf.reshape(qpos, [8])
    # qvel = tf.reshape(qvel, [14])
    cam_high = tf.reshape(cam_high, [256, 256, 3])
    cam_wrist = tf.reshape(cam_wrist, [256, 256, 3])
...
def dataset_generator_from_tfrecords(seed):
    tfrecord_path = '/home/silei/WorkSpace_git/RoboticsDiffusionTransformer/data/datasets/my_franka/tfrecords/'
    filepaths = []
    ...
    for filepath in filepaths:
        raw_dataset = tf.data.TFRecordDataset(filepath)  # 概念上: [915] - 915个序列化样本
        # map函数对数据集中的每个元素应用转换函数
        # dataset 由很多step的字典组成
        ...
        output_signature={
            'steps': tf.data.DatasetSpec(
                element_spec={
                    ...
                    'qpos': tf.TensorSpec(shape=(8), dtype=tf.float32),
                    # 'qvel': tf.TensorSpec(shape=(14), dtype=tf.float32),
                    'observation': {
                        'cam_high': tf.TensorSpec(shape=(256, 256, 3), dtype=tf.uint8),
                        'cam_wrist': tf.TensorSpec(shape=(256, 256, 3), dtype=tf.uint8),
                        # 'cam_left_wrist': tf.TensorSpec(shape=(256, 256, 3), dtype=tf.uint8),
                        # 'cam_right_wrist': tf.TensorSpec(shape=(256, 256, 3), dtype=tf.uint8),
                    },
